...
2. Отправляет данные в LLM с помощью методов классов LlmBankEmployee и LlmLawyer: сначала определяется категория письма, после заключение юриста, после отделы, которым необходимо перенаправить письмо. Исходя из полученных сведений и данных о предыдущем контексте переписки с данным отправителем генерируется ответ на письмо.
